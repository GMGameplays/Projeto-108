...
def countFingers(image, hand_landmarks, handNo=0):
    if hand_landmarks:
        ...
        for lm_index in tipIds:
            ...
            if landmarks[20].x > landmarks[4].x:
                if lm_index != 4:
                    if finger_tip_y < finger_bottom_y:
                        fingers.append(1)
                        print("Dedo com ID",lm_index,"esta aberto.")
                    if finger_tip_y > finger_bottom_y:
                        fingers.append(0)
                        print("Dedo com ID",lm_index,"esta fechado.")
                else:
                    if thumb_tip_x > thumb_bottom_x:
                        fingers.append(1)
                        print("O Polegar Esquerdo esta aberto.")
                    if thumb_tip_x < thumb_bottom_x:
                        fingers.append(0)
                        print("O Polegar Esquerdo esta aberto.")
            else:
            ...
            cv2.putText(image,text,(50,50),cv2.FONT_HERSHEY_SIMPLEX,1,(175,175,0))
